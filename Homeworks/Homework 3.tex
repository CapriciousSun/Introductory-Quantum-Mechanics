...
\begin{gather*}
|A - I| = \begin{bmatrix}
\cos(\theta) - 1 & \sin(\theta)e^{-i\varphi} \\
\sin(\theta)e^{i\varphi} & -\cos(\theta) - 1
\end{bmatrix} = \begin{bmatrix}
\frac{\cos(\theta) - 1}{\sin(\theta)e^{-i\varphi}} & 1 \\
1 & \frac{-\cos(\theta) - 1}{\sin(\theta)e^{i\phi}}
\end{bmatrix} = 
\end{gather*}
\color{black}
...
\color{violet}
The general function for solving some wavefunction at energy level $n$ is $\psi_{n}(x) = \frac{1}{\sqrt{2^{n}}n!}(y - \frac{d}{dy})^{n}\psi_{0}(y)$ where $y = \sqrt{\frac{m\omega}{\hbar}}x$. At the ground state, $\psi_{0}(x) = \left( \frac{m\omega}{\pi\hbar} \right)^{\frac{1}{4}}e^{-\frac{m\omega\pi^{2}}{2\hbar}}$
\color{black}
